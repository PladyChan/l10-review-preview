  ...
  <script>
    (() => {{
      ...
      const tools = [
        ...
        ["tools/l10-sharpness.html?v=20260721", "[data-l10-sharpness-link]", "[data-l10-sharpness-frame]"],
      ];
    ...
    (() => {{
      ...
      const syncToolFrames = (theme) => {{
        document.querySelectorAll("iframe[data-studio-tool-frame], iframe[data-day-sharpness-frame], iframe[data-l10-sharpness-frame]").forEach((frame) => {{
          try {{
            frame.contentWindow?.postMessage({{ type: "l10-theme", theme }}, window.location.origin);
          }} catch (error) {{}}
        }});
      }};
      ...
      window.addEventListener("storage", (event) => {{
        if (event.key !== "l10-theme") return;
        const saved = event.newValue;
        if (saved === "dark" || saved === "light") root.dataset.theme = saved;
        else delete root.dataset.theme;
        syncToolFrames(saved);
        render();
      }});
      render();
    }})();
